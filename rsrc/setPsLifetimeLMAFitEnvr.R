...
setPsLifetimeLMAFitEnvironment <- function()
{
                                         ...
                                         ... ) # optional: ReorderTaus (2-comp) or FixedTauDir (3-comp)
  {
    ...
    filt.lst <- if( ncol(LMATab) == 8 )
      ...
      .best.fit.env[["sortAndFilt3CompFits"]]( LMATab[,c(1:7,9:10)], args[["FixedTauDir"]], AddWeight, Verbose )
  ...
  # 1 - tau_dir is fixed, linear scale over all span (tau_dir must be fixed!)
                                                           ...
                                                           Verbose            = TRUE )
  {
    ...
    DIR.best <- .best.fit.env[["findBestByRSEFromFilteredTab"]](
      .best.fit.env[["sortAndFilt3CompFits"]]( lmfit.dir[,c(1:7,9:10)], FALSE, TRUE, Verbose ) ) # non-fixed Tau_dir, Add RSE weight, no verbose
    ...
    # II - log stage
    # !!! slow: executed twice !!!
    fit.name <- if( IRatioFixed ) "runSeededLMA3FreeTauDirLogIRatioFixedOMP"
                else "runSeededLMA3FreeTauDirLogOMP"
    ...
    RSE.best <- ..getBestFromUnfiltLMATab( lmfit.rse, TRUE, Verbose, FixedTauDir = FALSE ) # Add RSE weight, non-fixed Tau_dir
    KDE.best <- ..getBestFromUnfiltLMATab( lmfit.kde, TRUE, Verbose, FixedTauDir = FALSE ) # Add RSE weight, non-fixed Tau_dir
    
    # III (optional) - refine all parameters with fixed tau_oPS
    if( RefineWFixedTauoPs &
        is.finite(RSE.best[["BestFit"]][1]) &
        is.finite(KDE.best[["BestFit"]][1]) ) # ignore if NA's in the previous
    {
      fit.name <- if( IRatioFixed ) "runSeededLMA3TauoPsLinIRatioFixedOMP"
      ...
      lmfit.kde <- .lma.env[[fit.name]]( .pars.env[["ini.guess.df"]],
                                         ...
                                         DtRange = RefinedDtRange )
      RSE.bestRef <- ..getBestFromUnfiltLMATab( lmfit.rse, TRUE, Verbose, FixedTauDir = FALSE ) # Add RSE weight, non-fixed Tau_dir
      KDE.bestRef <- ..getBestFromUnfiltLMATab( lmfit.kde, TRUE, Verbose, FixedTauDir = FALSE ) # Add RSE weight, non-fixed Tau_dir
